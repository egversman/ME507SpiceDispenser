...
public:
    ...
    Node *remove(uint8_t);
    Node *pop();
    SpicyLinkedList *SpicyLinkedList::SpicySort(unordered_map<uint8_t, float>, 
                                                                       uint8_t);
     ...
     *  @param   startIdx
     */
    SpicyLinkedList *SpicyLinkedList::SpicySort(unordered_map<uint8_t, float> input, uint8_t startIdx)
    {
        if (input.size() == 0)
        {
            return;
